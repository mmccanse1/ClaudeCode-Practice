...
1b. **Breakfast and lunch generate the same dish on the same day.** Confirmed architectural gap — breakfast and lunch are generated by fully independent parallel calls with no cross-visibility, and both directives legitimately allow egg dishes, so they can converge. Three fix approaches sketched in 🆕 New section; needs a decision before implementation, so this is really "Tier 1 bug, Tier 4 decision" in one.
   - *Likely follow-up:* whichever fix lands, spot-check other diets for the same breakfast/lunch dish-space overlap (salads/bowls are a plausible second collision point), not just Home-Style/eggs.
...
- **Breakfast and lunch generate the same dish on the same day**
  - Owner screenshot: Friday breakfast = "Vegetable and Cheese Frittata with Roasted Sweet Potatoes," Friday lunch = "Vegetable Frittata with Herb Cheese" (Home-Style). Same dish concept, same day, different name.
  - **Root cause — confirmed, architectural, not a prompt typo.** In `claudeService.ts`, `generateMealPlan()` (line 961) fires one independent Claude call per meal type in parallel: `Promise.all(selected.map(meal => generateMealForType(...)))`. Breakfast and lunch have **zero visibility into each other's output** — each only sees its own 7-day list. Sides are the one exception (they're generated *after* mains and take the dinner list as pairing context); breakfast/lunch have no equivalent cross-check.
  - **Why frittata specifically, and why it's not rare:** this is PR #24's meal-appropriateness work working exactly as designed, just with a gap it didn't cover. The lunch directive (`MEAL_DIRECTIVE.lunch`, line 166) explicitly lists "frittatas or other egg dishes" as valid lunch fare, and Home-Style's breakfast directive (falls into the "full breakfast range" default, line 209) leans hard on eggs too. Both meal types independently and correctly reach for eggs — there's just no rule stopping them from landing on the *same* egg dish on the *same* day, because neither call knows the other is happening.
  - **This isn't Home-Style-specific or frittata-specific** — any diet whose breakfast and lunch directives overlap in dish-space (eggs are the obvious one, but salads/bowls could theoretically collide too) can hit this. Worth checking other diets for the same pattern before assuming it's a one-off.
  - **Fix directions to weigh Friday (not decided yet):**
    1. Sequential generation — run lunch after breakfast and pass breakfast's picks as context (mirrors how sides already take dinners as context). Simple, consistent with existing pattern, but sacrifices the parallel-call speed for the common case where no collision would've happened anyway.
    2. Post-hoc collision check — after the existing parallel generation, scan each day for cross-meal-type dish overlap and only regenerate the specific colliding recipe (reusing the existing single-recipe `regenerateRecipe` function). Keeps generation fast in the common case, only pays the extra-call cost when a collision actually occurs.
    3. Prompt-level rule addition — add an explicit "breakfast and lunch must not be the same dish concept" instruction to both directives without giving either call visibility into the other's actual output. Cheapest to add, but can't actually guarantee no collision since the calls still can't see each other — probably not sufficient on its own.
  - **Recommendation to open with Friday:** option 2 (post-hoc targeted regeneration) fits the app's existing patterns best and doesn't trade away generation speed for the common case, but worth deciding together rather than assuming.
